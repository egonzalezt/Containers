...
tags: [compose, docker, minecraft, multistage, network, volume]
---

We love to play Minecraft during vacations. To do this, we rent a cloud provider and use a virtual machine to host our vanilla Minecraft server or one with mods. However, we have always faced a problem: we need to use Linux screen to run our Minecraft server in the background, and if our server crashes, we have to manually restart it. Moreover, if the cloud provider is not accessible from our home, it becomes impossible to start the server again. For this reason, we have decided to dockerize our Minecraft server to address this situation.
